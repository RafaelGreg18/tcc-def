...
def get_selected_cid_training_energy(client_profile, selected_cids_time, client_dataset_size, model_size, epochs,
    ...
    for cid in selected_cids_time.keys():
        ...
        comm_joules_edge = model_size * client_profile[cid]["comm_joules"] * 4  # up+down to edge server
        ...
        if use_battery:
            ...
            cid_joule_consumption[cid] = (total, total_edge) # se delegar para o edge
...
def get_cid_training_carbon_footprint(client_profile, selected_cids_joules_consumption):
    ...
    for cid in selected_cids_joules_consumption.keys():
        ...
        try:
            carbon_footprint_train, carbon_footprint_comm = client_profile[cid]["device_carbon_intensity"] * (
                        selected_cids_joules_consumption[cid][0] / (3.6 * (10 ** 9))), client_profile[cid]["device_carbon_intensity"] * (selected_cids_joules_consumption[cid][1] / (3.6 * (10 ** 9))) # mJ -> kWh
            # TODO: pegada de carbono de idle??
            cid_carbon_footprint[cid] = (carbon_footprint_train, carbon_footprint_comm)
        except TypeError:
            carbon_footprint_idle = (client_profile[cid]["device_carbon_intensity"] * (
                        selected_cids_joules_consumption[cid] / (3.6 * (10 ** 9))))
            cid_carbon_footprint[cid] = carbon_footprint_idle
