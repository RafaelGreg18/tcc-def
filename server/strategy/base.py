...
class BaseStrategy(Strategy):
    def __init__(self, *, repr: str, num_clients: int, profiles: dict, num_participants: int, num_evaluators: int,
        ...
        # TODO trocar o path com algum colocado no pyproject
    ...
    def update_cids_current_battery(self, cids_joules_consumption):
        for cid in cids_joules_consumption.keys():
            ...
            if type(cid_joules_consumption) is float:
                self.profiles[cid]["current_battery_mJ"] -= max(cid_joules_consumption,0)
                continue
    ...
    def save_round_system_metrics(self, all_cids_carbon_footprint,all_cids_joules_consumption,
        ...
        selected_mj, unselected_mj = 0, 0
        selected_ceq, unselected_ceq = 0, 0
        total_mJ, total_ceq = 0, 0
        for cid, energy_consumption in all_cids_joules_consumption.items():
            try:
                if self.profiles[cid]["current_battery_mJ"] < self.context.run_config["battery-threshold"] and self.profiles[cid]['has_edge']:
                    selected_mj += energy_consumption[1]
                else:
                    selected_mj += energy_consumption[0]
            except TypeError:
                unselected_mj += energy_consumption
        total_mJ = selected_mj + unselected_mj

        for cid, carbon_footprint in selected_cids_carbon_footprint.items():
            if self.profiles[cid]["current_battery_mJ"] < self.context.run_config["battery-threshold"] and self.profiles[cid]['has_edge']:
                selected_ceq += carbon_footprint[1]
            else:
                selected_ceq += carbon_footprint[0]

        for cid, carbon_footprint in unselected_cids_carbon_footprint.items():
            unselected_ceq += carbon_footprint

        total_ceq = selected_ceq + unselected_ceq

        my_results = {"total_mJ": total_mJ,
                      "total_ceq": total_ceq,
                      "selected_mJ": selected_mj,
                      "unselected_mJ": unselected_mj,
                      "selected_ceq": selected_ceq,
                      "unselected_ceq": unselected_ceq,
