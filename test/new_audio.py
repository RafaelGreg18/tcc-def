...
def build_fds_train_test():
    pre = None  # (opcional) poderíamos filtrar aqui faremos o filtro no carregamento
...
def main():
    ...
    for rnd in range(1, CFG.rounds + 1):
        ...
        for pid in chosen:
            ...
            # filtra a partição do cliente para KWS10
            part_kws = filter_to_kws10(part, label_names)
            if len(part_kws) == 0:
                continue  # cliente sem KWS10 pós-filtro
...
# def filter_kws10_on_test(test_ds: Dataset) -> Dataset:
#     """
#     Mantém somente exemplos do split 'test' cujas labels pertencem a KWS10.
#     Também remove itens marcados como 'is_unknown'.
#     """
#     # Mapeia nomes de classe -> ids permitidos (usando ClassLabel.names)
#     label_names = test_ds.features["label"].names
#     allow_ids = {i for i, name in enumerate(label_names) if name.lower() in KWS10}
#
#     def _keep(ex):
#         return (not ex.get("is_unknown", False)) and int(ex["label"]) in allow_ids
#
#     return test_ds.filter(_keep)
#
# def find_partitions_with_outside_kws10(fds: FederatedDataset, split: str = "train") -> Dict[int, Dict]:
#     """
#     Retorna um dicionário {partition_id: info} apenas para as partições que têm amostras
#     fora de KWS10 (ou marcadas como is_unknown=True). 'info' inclui a contagem total e
#     o breakdown por label-id e label-name.
#     """
#     # 1) Precisamos das labels (nomes) do split completo para mapear nome<->id
#     ds_full = fds.load_split(split)  # HF Dataset do split (não particionado)
#     label_names: List[str] = ds_full.features["label"].names
#     allow_ids = {i for i, n in enumerate(label_names) if n.lower() in KWS10}
#
#     # 2) Garantir que o partitioner esteja inicializado; depois ler nº de partições
#     _ = fds.load_partition(0, split=split)  # "warm-up"
#     n_parts = fds.partitioners[split].num_partitions
#
#     offenders: Dict[int, Dict] = {}
#
#     # 3) Percorrer partições e checar “fora de KWS10”
#     for pid in range(n_parts):
#         part = fds.load_partition(pid, split=split)  # HF Dataset com apenas os exemplos do cliente pid
#
#         # Filtra amostras fora de KWS10 ou marcadas como 'is_unknown'
#         def is_outside(ex):
#             return ex["is_unknown"] or (int(ex["label"]) not in allow_ids)
#
#         bad = part.filter(is_outside)
#         if bad.num_rows > 0:
#             # Contagem por label-id e label-name
#             counts = Counter(bad["label"])
#             counts_by_name = {label_names[i]: c for i, c in counts.items()}
#             offenders[pid] = {
#                 "total_outside": bad.num_rows,
#                 "by_label_id": dict(counts),
#                 "by_label_name": counts_by_name,
#             }
#
#     return offenders
#
...
#     part_train = NaturalIdPartitioner(partition_by="speaker_id")  # clientes = speakers únicos no 'train' filtrado
#
#     fds = FederatedDataset(
#         dataset="google/speech_commands",
#         subset="v0.02",
#         preprocessor=pre,                          # << filtro roda antes do particionamento
#         partitioners={"train": part_train},        # particiona só o train
#         trust_remote_code=True,
#         download_config=DownloadConfig(resume_download=True, max_retries=20, storage_options={"timeout": 600}),
#     )
#     return fds
#
# def main():
...
#     num_clients = len(train.unique("speaker_id"))
#     print("nº de clientes (train):", num_clients)
#
#     # --------- Verificando se houve filtro ---------
#     offenders = find_partitions_with_outside_kws10(fds, split="train")
#     if not offenders:
#         print("Nenhuma partição contém amostras fora de KWS10.")
#     else:
#         print(f"{len(offenders)} partições com amostras fora de KWS10:")
#         for pid, info in offenders.items():
#             print(f"- partição {pid}: {info['total_outside']} amostras (por label: {info['by_label_name']})")
#
#     # --------- Verificando features de teste ---------
#     test = fds.load_split("test")  # HF Dataset do split (não particionado)
#     print(f"Total no test: {test.num_rows}")
#     test = filter_kws10_on_test(test)  # somente KWS10
#     print("Total no test (KWS10):", test.num_rows)
#
# if __name__ == '__main__':
#     main()
