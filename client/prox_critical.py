...
from utils.model.manipulation import set_weights, get_weights, train_prox_critical


class ProxCriticalClient(BaseProxClient):
    def __init__(self, cid, flwr_cid, model, dataloader, dataset_id, proximal_mu, **kwargs):
        super().__init__(cid, flwr_cid, model, dataloader, dataset_id, proximal_mu, **kwargs)
    ...
    def fit(
        ...
        if int(config["server_round"]) == 1:
            ...
            avg_loss, avg_acc, stat_util, gn = train_prox_critical(self.model, self.dataloader, epochs, criterion,
